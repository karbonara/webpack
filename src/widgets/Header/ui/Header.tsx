export const Header = () => {
  return (
    <>
      <a className="fixed-cart js-cart-icon " href="/basket" />
      <header className="b-header js-header">
        <div className="b-header__wrapper">
          <div className="b-header__logo">
            <a href="/">
              <svg
                width={132}
                height={53}
                viewBox="0 0 132 53"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M66.1259 13.995C66.1259 8.87506 69.2859 5.69189 73.351 5.69189C76.8495 5.69189 79.3288 8.19241 79.3288 9.67277C79.3327 10.0606 79.2125 10.4393 78.986 10.7527C78.7595 11.0661 78.4388 11.2974 78.0716 11.4124C77.7044 11.5274 77.3101 11.5199 76.9475 11.3909C76.5848 11.262 76.2731 11.0186 76.0585 10.6968C75.3816 9.78783 74.7048 9.10517 73.351 9.10517C71.4496 9.10517 69.7422 10.8118 69.7422 13.995C69.7422 17.1782 71.4344 18.8886 73.351 18.8886C74.014 18.8659 74.6533 18.6344 75.1792 18.2265C75.705 17.8186 76.091 17.2548 76.2829 16.6144H73.9138C73.682 16.6301 73.4495 16.5976 73.2307 16.5189C73.0119 16.4402 72.8115 16.3171 72.6418 16.1571C72.4722 15.997 72.3369 15.8036 72.2445 15.5887C72.152 15.3738 72.1043 15.142 72.1043 14.9077C72.1043 14.6735 72.152 14.4417 72.2445 14.2268C72.3369 14.0119 72.4722 13.8185 72.6418 13.6584C72.8115 13.4984 73.0119 13.3753 73.2307 13.2966C73.4495 13.2179 73.682 13.1854 73.9138 13.2011H78.2032C78.666 13.1887 79.115 13.361 79.4525 13.6805C79.79 14.0001 79.9888 14.4411 80.0057 14.9077V15.019C80.0057 19.4562 77.1879 22.3057 73.351 22.3057C69.2859 22.3019 66.1259 19.1149 66.1259 13.995Z"
                  fill="white"
                />
                <path
                  d="M89.8014 19.115H85.0632L84.6145 20.4804C84.4959 20.9086 84.2289 21.2796 83.8619 21.526C83.4949 21.7724 83.0525 21.8778 82.615 21.823C82.1775 21.7682 81.7741 21.5569 81.4781 21.2274C81.182 20.8979 81.0131 20.4723 81.002 20.0278C81.0411 19.5573 81.1553 19.0962 81.3404 18.6625L85.4017 7.39867C85.542 6.96718 85.8139 6.59141 86.1786 6.325C86.5433 6.0586 86.982 5.91516 87.4323 5.91516C87.8826 5.91516 88.3214 6.0586 88.686 6.325C89.0507 6.59141 89.3226 6.96718 89.463 7.39867L93.5243 18.6625C93.7093 19.0962 93.8236 19.5573 93.8627 20.0278C93.8547 20.4737 93.6875 20.9017 93.3918 21.2332C93.0961 21.5647 92.6917 21.7775 92.2529 21.8326C91.8141 21.8876 91.3704 21.7811 91.0032 21.5327C90.636 21.2843 90.3699 20.9107 90.2539 20.4804L89.8014 19.115ZM87.5464 12.0622H87.322L86.0786 15.9319H88.7861L87.5464 12.0622Z"
                  fill="white"
                />
                <path
                  d="M100.89 6.14067C104.499 6.14067 106.374 8.05824 106.374 10.3593C106.374 12.7486 104.681 13.3162 104.681 13.3162V13.5463C104.681 13.5463 107.724 13.9989 107.724 17.3009C107.724 19.602 105.579 21.8494 101.97 21.8494H97.5704C97.1079 21.8609 96.6595 21.6882 96.3222 21.3689C95.9849 21.0495 95.7858 20.609 95.7679 20.1428V7.82813C95.7897 7.36492 95.9906 6.92876 96.3276 6.61327C96.6646 6.29778 97.1108 6.1281 97.5704 6.14067H100.89ZM101.183 12.0583C101.356 12.075 101.531 12.0551 101.696 11.9998C101.861 11.9445 102.012 11.855 102.141 11.7371C102.27 11.6192 102.372 11.4755 102.443 11.3152C102.513 11.1548 102.549 10.9814 102.549 10.8061C102.549 10.6308 102.513 10.4574 102.443 10.2971C102.372 10.1367 102.27 9.99302 102.141 9.87512C102.012 9.75721 101.861 9.66773 101.696 9.61243C101.531 9.55712 101.356 9.5372 101.183 9.55395H99.3767V12.0583H101.183ZM102.084 15.4716H99.3767V18.4285H102.084C103.225 18.4285 103.891 17.7458 103.891 16.9519C103.891 16.1581 103.214 15.4754 102.084 15.4754V15.4716Z"
                  fill="white"
                />
                <path
                  d="M118.767 18.4323C119.23 18.4198 119.679 18.592 120.017 18.9115C120.355 19.231 120.555 19.672 120.573 20.1389C120.541 20.6024 120.337 21.0368 120.003 21.3563C119.668 21.6758 119.227 21.8572 118.767 21.8647H111.773C111.31 21.8772 110.861 21.705 110.523 21.3855C110.185 21.0661 109.985 20.625 109.967 20.1581V7.88564C109.971 7.65112 110.021 7.41972 110.114 7.20479C110.207 6.98986 110.342 6.79567 110.51 6.63342C110.678 6.47117 110.876 6.34409 111.093 6.25951C111.31 6.17493 111.541 6.13453 111.773 6.14065H118.645C119.108 6.12819 119.557 6.30037 119.896 6.61984C120.234 6.93932 120.433 7.38035 120.451 7.84729C120.434 8.3149 120.235 8.7569 119.897 9.0772C119.559 9.3975 119.109 9.5702 118.645 9.55776H113.576V12.2769H117.188C117.42 12.2612 117.653 12.2937 117.872 12.3724C118.09 12.451 118.291 12.5742 118.46 12.7342C118.63 12.8942 118.765 13.0877 118.858 13.3026C118.95 13.5175 118.998 13.7493 118.998 13.9835C118.998 14.2178 118.95 14.4496 118.858 14.6645C118.765 14.8794 118.63 15.0728 118.46 15.2328C118.291 15.3928 118.09 15.516 117.872 15.5947C117.653 15.6733 117.42 15.7058 117.188 15.6902H113.576V18.4208L118.767 18.4323Z"
                  fill="white"
                />
                <path
                  d="M66.126 33.8802C66.126 31.3796 68.3848 29.1016 71.4497 29.1016C74.7201 29.1016 76.8648 31.2646 76.8648 32.6299C76.8469 33.0968 76.6473 33.5379 76.3092 33.8573C75.971 34.1768 75.5217 34.349 75.0585 34.3365C74.8008 34.3514 74.543 34.3066 74.3051 34.2056C74.0672 34.1045 73.8553 33.9499 73.6857 33.7536C73.1229 33.1822 72.4461 32.4995 71.541 32.4995C70.5257 32.4995 69.8488 33.1822 69.8488 33.9799C69.8488 36.3692 77.0739 35.4602 77.0739 40.9215C77.0739 43.4028 74.8189 45.7001 71.4764 45.7001C68.0539 45.7001 65.9054 43.537 65.9054 42.1717C65.9232 41.7048 66.1229 41.2638 66.461 40.9443C66.7991 40.6248 67.2485 40.4526 67.7117 40.4651C67.965 40.4521 68.218 40.4963 68.4522 40.5945C68.6864 40.6927 68.8959 40.8424 69.0654 41.0327C69.6282 41.6041 70.3051 42.2868 71.4345 42.2868C72.6742 42.2868 73.3359 41.6041 73.3359 40.8064C73.3473 38.4325 66.126 39.3414 66.126 33.8802Z"
                  fill="white"
                />
                <path
                  d="M82.2227 32.9866H79.9639C79.7322 33.0023 79.4997 32.9698 79.2809 32.8911C79.0621 32.8124 78.8617 32.6893 78.692 32.5292C78.5223 32.3692 78.3871 32.1758 78.2946 31.9609C78.2021 31.746 78.1544 31.5142 78.1544 31.2799C78.1544 31.0457 78.2021 30.8139 78.2946 30.599C78.3871 30.3841 78.5223 30.1906 78.692 30.0306C78.8617 29.8706 79.0621 29.7475 79.2809 29.6688C79.4997 29.5901 79.7322 29.5576 79.9639 29.5733H88.0865C88.3183 29.5576 88.5508 29.5901 88.7696 29.6688C88.9884 29.7475 89.1888 29.8706 89.3585 30.0306C89.5281 30.1906 89.6634 30.3841 89.7558 30.599C89.8483 30.8139 89.896 31.0457 89.896 31.2799C89.896 31.5142 89.8483 31.746 89.7558 31.9609C89.6634 32.1758 89.5281 32.3692 89.3585 32.5292C89.1888 32.6893 88.9884 32.8124 88.7696 32.8911C88.5508 32.9698 88.3183 33.0023 88.0865 32.9866H85.8315V43.5677C85.8102 44.0359 85.6108 44.4778 85.2747 44.8015C84.9387 45.1252 84.4918 45.3059 84.0271 45.3059C83.5624 45.3059 83.1156 45.1252 82.7795 44.8015C82.4435 44.4778 82.2441 44.0359 82.2227 43.5677V32.9866Z"
                  fill="white"
                />
                <path
                  d="M97.6654 45.7154C93.7182 45.7154 90.4403 42.4134 90.4403 37.4085C90.4403 32.4036 93.7106 29.1016 97.6654 29.1016C101.62 29.1016 104.891 32.4036 104.891 37.4085C104.891 42.4134 101.616 45.7154 97.6654 45.7154ZM97.6654 32.5148C95.7641 32.5148 94.0567 34.2215 94.0567 37.4085C94.0567 40.5955 95.7489 42.3021 97.6654 42.3021C99.582 42.3021 101.278 40.5955 101.278 37.4085C101.278 34.2215 99.5858 32.5148 97.6654 32.5148Z"
                  fill="white"
                />
                <path
                  d="M110.99 43.5524C110.969 44.0206 110.769 44.4624 110.433 44.7862C110.097 45.1099 109.65 45.2905 109.186 45.2905C108.721 45.2905 108.274 45.1099 107.938 44.7862C107.602 44.4624 107.403 44.0206 107.381 43.5524V31.2799C107.399 30.813 107.599 30.3719 107.937 30.0525C108.275 29.733 108.724 29.5608 109.188 29.5733H112.796C116.744 29.5733 118.664 31.4908 118.664 34.559C118.664 37.5197 116.633 38.5437 116.633 38.5437L118.535 42.0682C118.811 42.5131 118.968 43.0236 118.987 43.5485C118.969 44.0155 118.77 44.4565 118.432 44.776C118.093 45.0955 117.644 45.2676 117.181 45.2552C116.861 45.2598 116.546 45.178 116.269 45.0181C115.991 44.8582 115.761 44.6262 115.603 44.3462L113.12 39.5677H110.975L110.99 43.5524ZM112.796 36.1582C114.15 36.1582 114.827 35.4756 114.827 34.5628C114.827 33.65 114.15 32.9865 112.796 32.9865H110.99V36.1735L112.796 36.1582Z"
                  fill="white"
                />
                <path
                  d="M130.194 41.8458C130.657 41.8334 131.106 42.0056 131.445 42.325C131.783 42.6445 131.982 43.0855 132 43.5525C131.963 44.0125 131.758 44.4422 131.424 44.7577C131.09 45.0733 130.651 45.252 130.194 45.2591H123.208C122.745 45.2716 122.296 45.0994 121.958 44.7799C121.619 44.4604 121.42 44.0194 121.402 43.5525V31.28C121.416 30.8104 121.614 30.3654 121.952 30.0427C122.291 29.7199 122.743 29.5456 123.208 29.558H130.087C130.319 29.5424 130.552 29.5749 130.77 29.6535C130.989 29.7322 131.19 29.8554 131.359 30.0154C131.529 30.1754 131.664 30.3688 131.757 30.5837C131.849 30.7986 131.897 31.0304 131.897 31.2647C131.897 31.4989 131.849 31.7307 131.757 31.9456C131.664 32.1605 131.529 32.354 131.359 32.514C131.19 32.674 130.989 32.7972 130.77 32.8758C130.552 32.9545 130.319 32.987 130.087 32.9713H125.003V35.7019H128.616C128.847 35.6863 129.08 35.7188 129.299 35.7974C129.518 35.8761 129.718 35.9993 129.888 36.1593C130.057 36.3193 130.193 36.5127 130.285 36.7276C130.378 36.9425 130.425 37.1743 130.425 37.4086C130.425 37.6428 130.378 37.8746 130.285 38.0895C130.193 38.3044 130.057 38.4979 129.888 38.6579C129.718 38.8179 129.518 38.9411 129.299 39.0197C129.08 39.0984 128.847 39.1309 128.616 39.1152H125.003V41.8458H130.194Z"
                  fill="white"
                />
                <path
                  d="M32.3692 37.0135C31.0093 35.6421 30.0831 33.8948 29.7079 31.9925C29.3326 30.0902 29.5252 28.1184 30.2611 26.3265C30.997 24.5345 32.2433 23.0029 33.8423 21.9253C35.4414 20.8477 37.3213 20.2726 39.2445 20.2726C41.1677 20.2726 43.0476 20.8477 44.6467 21.9253C46.2457 23.0029 47.492 24.5345 48.2279 26.3265C48.9639 28.1184 49.1564 30.0902 48.7811 31.9925C48.4059 33.8948 47.4797 35.6421 46.1198 37.0135H32.3692Z"
                  fill="white"
                />
                <path
                  d="M12.9299 37.0135C11.57 35.6421 10.6438 33.8948 10.2686 31.9925C9.89331 30.0902 10.0858 28.1184 10.8218 26.3265C11.5577 24.5345 12.804 23.0029 14.403 21.9253C16.002 20.8477 17.882 20.2726 19.8052 20.2726C21.7283 20.2726 23.6083 20.8477 25.2073 21.9253C26.8064 23.0029 28.0527 24.5345 28.7886 26.3265C29.5245 28.1184 29.717 30.0902 29.3418 31.9925C28.9665 33.8948 28.0404 35.6421 26.6805 37.0135H12.9299Z"
                  fill="white"
                />
                <path
                  d="M55.1091 15.025C52.6632 10.4457 49.0134 6.63467 44.5605 4.01047C39.9814 1.31581 34.761 -0.0697132 29.4599 0.00273948C24.1576 -0.0702181 18.9358 1.31532 14.3556 4.01047C9.90123 6.63411 6.25004 10.4452 3.80311 15.025C1.26797 19.6962 0.000405794 24.9542 0.000405794 30.799C-0.0185109 34.8028 0.624068 38.7819 1.90176 42.5729C3.07548 46.13 4.9323 49.4192 7.36624 52.2528L29.4599 50.9795V42.1011H23.8928V46.4885L9.92926 47.2824C6.95174 43.1098 5.46108 37.0579 5.46108 31.0674C5.39434 26.6087 6.47806 22.2091 8.60592 18.3002C10.6223 14.589 13.5956 11.4962 17.2114 9.34899C20.9244 7.16028 25.1596 6.03576 29.4599 6.09679C33.7718 6.03981 38.0181 7.16381 41.7465 9.34899C45.3796 11.4824 48.3577 14.5868 50.352 18.3194C52.4513 22.2543 53.5194 26.6647 53.455 31.1326C53.455 38.2506 51.1936 44.1197 46.671 48.7398L50.5611 52.5749C53.3494 49.9701 55.4899 46.7392 56.809 43.1443C58.248 39.2174 58.962 35.058 58.9156 30.8718C58.9156 24.9836 57.6468 19.7013 55.1091 15.025Z"
                  fill="white"
                />
                <g className="eye">
                  <path
                    d="M21.4442 30.6318C22.5321 30.6318 23.414 29.7424 23.414 28.6452C23.414 27.548 22.5321 26.6586 21.4442 26.6586C20.3563 26.6586 19.4744 27.548 19.4744 28.6452C19.4744 29.7424 20.3563 30.6318 21.4442 30.6318Z"
                    fill="black"
                  />
                </g>
                <g className="eye">
                  <path
                    d="M41.2143 30.6318C42.3022 30.6318 43.1841 29.7424 43.1841 28.6452C43.1841 27.548 42.3022 26.6586 41.2143 26.6586C40.1264 26.6586 39.2445 27.548 39.2445 28.6452C39.2445 29.7424 40.1264 30.6318 41.2143 30.6318Z"
                    fill="black"
                  />
                </g>
              </svg>
            </a>
          </div>
          <div className="b-header__container b-header__container--search">
            <div
              className="btn btn--header sm-hide js-modal-open"
              data-modal="menu"
            >
              Каталог игр
            </div>
            <div id="search" />
          </div>
          <div className="b-header__icons b-header__container">
            <div
              className="btn btn--header sm-hide js-modal-open"
              data-modal="login"
            >
              Войти
            </div>
            <a
              className="b-header__icon b-header__icon--search js-modal-open"
              data-modal="search"
            />
            <a
              className="b-header__icon b-header__icon--favorite sm-hide js-modal-open"
              data-modal="login"
            />
            <a
              className="b-header__icon b-header__icon--cart sm-hide js-cart-icon "
              href="/basket"
            />
            <div className="b-header__menu-btn js-modal-open" data-modal="menu">
              <span />
              <span />
              <span />
            </div>
          </div>
          <div className="b-popup js-modal hide b-popup--menu" data-modal="menu">
            <span className="b-popup__overlay" />
            <div className="b-popup__container js-custom-scrollbar">
              <div className="b-popup__content js-modal-content">
                <div className="b-menu__close js-menu-close" />
                <div className="b-menu__icons">
                  <div className="btn btn--header js-modal-open" data-modal="login">
                    Войти
                  </div>{" "}
                  <a
                    className="b-header__icon b-header__icon--favorite js-modal-open"
                    data-modal="login"
                  />
                  <a
                    className="b-header__icon b-header__icon--cart js-cart-icon "
                    href="/basket"
                  />
                </div>
                <div
                  className="b-menu__wrapper js-accordeon-container"
                  data-mobile="true"
                >
                  <ul className="b-menu__list b-menu__list--main b-menu__col">
                    <li className="b-menu__list-item b-menu__list-item--display--mobile">
                      {" "}
                      <a href="/catalog">все игры</a>
                    </li>
                    <li className="b-menu__list-item">
                      {" "}
                      <a href="/top">лидеры продаж</a>
                    </li>
                    <li className="b-menu__list-item">
                      {" "}
                      <a href="/newgames">новинки</a>
                    </li>
                    <li className="b-menu__list-item">
                      {" "}
                      <a href="/preorders">предзаказы</a>
                    </li>
                    <li className="b-menu__list-item">
                      {" "}
                      <a href="/sale">скидки</a>
                    </li>
                    <li className="b-menu__list-item">
                      {" "}
                      <a href="/collections">подборки </a>
                    </li>
                    <ul className="b-menu__list b-menu__list-sub">
                      <li className="b-menu__list-item">
                        {" "}
                        <a href="/page/company">О компании</a>
                      </li>
                      <li className="b-menu__list-item">
                        {" "}
                        <a href="/page/support">Поддержка</a>
                      </li>
                    </ul>
                  </ul>
                  <div className="b-menu__accordeon b-menu__col">
                    <div
                      className="b-menu__accordeon-title js-accordeon-open"
                      data-mobile="true"
                    >
                      жанры
                    </div>
                    <ul className="b-menu__accordeon-list">
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=1">
                          Экшен
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=22">
                          Ролевые
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=2">
                          Приключения
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=24">
                          Стратегии
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=23">
                          Симуляторы
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=25">
                          Онлайн
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=705">
                          Инди
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=27">
                          Спорт
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=706">
                          Казуальные
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/catalog?ProductFilter%5Bgenre%5D%5B%5D=1467">
                          Гонки
                        </a>
                      </li>
                      <ul className="b-menu__list b-menu__list-sub">
                        <li className="b-menu__list-item">
                          {" "}
                          <a href="/catalog">Все игры — 3246</a>
                        </li>
                      </ul>
                    </ul>
                  </div>
                  <div className="b-menu__accordeon b-menu__col">
                    <div
                      className="b-menu__accordeon-title js-accordeon-open"
                      data-mobile="true"
                    >
                      Издатели
                    </div>
                    <ul className="b-menu__accordeon-list">
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/2K">2K</a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/BANDAI-NAMCO">
                          Bandai Namco Entertainment
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/Bethesda-Softworks">
                          Bethesda Softworks
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/Capcom">Capcom</a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/Electronic-Arts">Electronic Arts</a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/Paradox-Interactive">
                          Paradox Interactive
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/SEGA">SEGA</a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/publishers/Ubisoft">Ubisoft</a>
                      </li>
                      <ul className="b-menu__list b-menu__list-sub">
                        <li className="b-menu__list-item">
                          <a href="/publishers">Все издатели — 353</a>
                        </li>
                      </ul>
                    </ul>
                  </div>
                  <div className="b-menu__accordeon b-menu__col">
                    <div
                      className="b-menu__accordeon-title js-accordeon-open"
                      data-mobile="true"
                    >
                      разработчики
                    </div>
                    <ul className="b-menu__accordeon-list">
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/developers/Creative-Assembly">
                          Creative Assembly
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/developers/DICE">DICE</a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/developers/Gearbox-Software">Gearbox Software</a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/developers/Hangar-13">Hangar 13</a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/developers/Remedy-Entertainment">
                          Remedy Entertainment
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/developers/Respawn-Entertainment">
                          Respawn Entertainment
                        </a>
                      </li>
                      <li className="b-menu__accordeon-item">
                        {" "}
                        <a href="/developers/Team17-Digital">Team17 Digital</a>
                      </li>
                      <ul className="b-menu__list b-menu__list-sub">
                        <li className="b-menu__list-item">
                          <a href="/developers">Все разработчики — 1438</a>
                        </li>
                      </ul>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="b-popup js-modal hide b-popup--small" data-modal="login">
            <span className="b-popup__overlay" />
            <div className="b-popup__container js-custom-scrollbar b-popup__container--login">
              <div className="b-popup__content js-modal-content">
                <div className="b-login">
                  <div className="b-login__close js-menu-close" />
                  <div className="b-lobin__wrapper">
                    <div className="b-tabs b-tabs--center js-tab">
                      <div className="b-tabs__head">
                        <div
                          className="b-tabs__head-item js-tab-button active"
                          data-tab-index={1}
                        >
                          Вход
                        </div>
                        <div
                          className="b-tabs__head-item js-tab-button"
                          data-tab-index={2}
                        >
                          Регистрация
                        </div>
                      </div>
                      <div className="b-tabs__body">
                        <div className="js-tab-content active" data-tab-index={1}>
                          <form className="b-form b-form--signup js-form js-form-login">
                            <input
                              type="hidden"
                              name="_csrf"
                              defaultValue="b09OaVkwQXhaIDouA1gTNgd.HzY4fS1OMDcROQEBCkAwDSs2NmcuNQ=="
                            />{" "}
                            <input
                              type="hidden"
                              name="LoginForm[grecaptchatoken]"
                              defaultValue=""
                              id="loginform-grecaptchatoken"
                            />
                            <input type="hidden" name="referrer" />
                            <meta name="csrf-param" content="_csrf" />
                            <meta
                              name="csrf-token"
                              content="b09OaVkwQXhaIDouA1gTNgd.HzY4fS1OMDcROQEBCkAwDSs2NmcuNQ=="
                            />
                            <div className="b-form__message js-message" />
                            <div className="b-form__input">
                              <input
                                type="text"
                                id="loginform-username"
                                className="input input--lg input--ta-cent"
                                name="LoginForm[username]"
                                data-validate="email"
                              />{" "}
                              <label>E-mail</label>
                              <div className="b-form__input-message" />
                            </div>
                            <div className="b-form__input">
                              <input
                                type="password"
                                id="loginform-password"
                                className="input input--lg input--ta-cent"
                                name="LoginForm[password]"
                                data-validate="password"
                              />
                              <div className="b-form__input-message" />
                              <label>Пароль</label>
                            </div>
                            <div
                              style={{
                                fontSize: 10,
                                marginTop: "-15px",
                                marginBottom: 20
                              }}
                            >
                              Этот сайт защищен reCAPTCHA и применяются{" "}
                              <a href="https://policies.google.com/privacy">
                                Политика конфиденциальности
                              </a>
                              и{" "}
                              <a href="https://policies.google.com/terms">
                                Условия использования
                              </a>{" "}
                              Google.
                            </div>
                            <button type="submit" className="btn btn--primary">
                              авторизоваться
                            </button>
                          </form>
                          <form
                            id="w1"
                            className="b-form b-form--signup js-form js-form-resetpw hide"
                            action="/"
                            method="post"
                          >
                            <input
                              type="hidden"
                              name="_csrf"
                              defaultValue="b09OaVkwQXhaIDouA1gTNgd.HzY4fS1OMDcROQEBCkAwDSs2NmcuNQ=="
                            />
                            <div className="b-form__resetpassord">
                              <div className="b-form__resetpassord-title">
                                Забыли пароль?
                              </div>
                              <div className="b-form__resetpassord-description">
                                Мы пришлем пароль на использованный при регистрации
                                ящик
                              </div>
                            </div>
                            <div className="b-form__input">
                              <input
                                type="text"
                                id="passwordresetrequestform-email"
                                className="input input--lg input--ta-cent"
                                name="PasswordResetRequestForm[email]"
                                data-validate="email"
                              />{" "}
                              <label>E-mail</label>
                              <div className="b-form__input-message" />
                            </div>
                            <button type="submit" className="btn btn--primary">
                              отправить
                            </button>
                          </form>
                          <div className="b-form__resetpassord-toggle">
                            <p>Забыли пароль? </p>
                            <span className="js-toggle-forms">Восстановить</span>
                          </div>
                          <div className="b-autch-social">
                            <div className="b-autch-social__title">
                              Войти через привязанный аккаунт
                            </div>
                            <div className="b-autch-social__wrapper">
                              {" "}
                              <a
                                className="b-autch-social__item b-autch-social__item--vk"
                                href="/login?service=vk"
                              >
                                <svg
                                  width={31}
                                  height={18}
                                  viewBox="0 0 31 18"
                                  fill="none"
                                  xmlns="http://www.w3.org/2000/svg"
                                >
                                  <path
                                    d="M25.9588 11.2326C26.9539 12.243 28.0068 13.1939 28.9002 14.3087C29.296 14.8019 29.669 15.3119 29.953 15.8854C30.3584 16.7033 29.9923 17.6002 29.2881 17.6488L24.9138 17.6479C23.7841 17.745 22.885 17.2714 22.1271 16.4685C21.522 15.8287 20.9606 15.1452 20.3779 14.4836C20.1398 14.212 19.889 13.9564 19.5902 13.7552C18.9939 13.3519 18.4758 13.4754 18.1341 14.123C17.7859 14.7819 17.7064 15.5122 17.6732 16.2456C17.6256 17.3177 17.3146 17.5979 16.2796 17.6483C14.0682 17.7559 11.97 17.4072 10.0198 16.2483C8.29948 15.2261 6.96792 13.7833 5.80761 12.1499C3.54816 8.96562 1.81775 5.47074 0.262955 1.8755C-0.0869721 1.06581 0.169029 0.632584 1.02834 0.615782C2.456 0.587172 3.88367 0.590805 5.31134 0.614419C5.89237 0.623956 6.2768 0.969538 6.50004 1.53945C7.27154 3.51259 8.21735 5.3899 9.40256 7.13099C9.71841 7.59464 10.0408 8.05693 10.5 8.38435C11.0067 8.74582 11.3929 8.62639 11.6319 8.03831C11.7848 7.66457 11.8507 7.26495 11.8839 6.86442C11.9975 5.49254 12.0111 4.1211 11.8145 2.75467C11.6922 1.89957 11.2295 1.34736 10.4095 1.1857C9.99188 1.1035 10.053 0.942745 10.2562 0.694798C10.6087 0.266112 10.939 0.000908232 11.5991 0.000908232L16.5413 0C17.3203 0.158941 17.495 0.522234 17.6007 1.33783L17.6051 7.04698C17.5959 7.36259 17.7576 8.29807 18.3028 8.50469C18.7396 8.65455 19.0279 8.29035 19.2892 8.00289C20.4744 6.69549 21.3189 5.1524 22.0751 3.55528C22.4088 2.85094 22.6967 2.12209 22.9763 1.39187C23.1843 0.851922 23.5075 0.586264 24.0938 0.595346L28.853 0.60125C28.9932 0.60125 29.1356 0.602612 29.2746 0.627589C30.0766 0.770181 30.2964 1.12893 30.0482 1.94226C29.6577 3.22014 28.8989 4.28459 28.1571 5.35176C27.362 6.49432 26.514 7.59691 25.7268 8.74446C25.0034 9.79347 25.0606 10.3216 25.9588 11.2326Z"
                                    fill="white"
                                  />
                                </svg>
                              </a>
                              <a
                                className="b-autch-social__item b-autch-social__item--google"
                                href="/login?service=google"
                              >
                                <svg
                                  width={24}
                                  height={24}
                                  viewBox="0 0 24 24"
                                  fill="none"
                                  xmlns="http://www.w3.org/2000/svg"
                                >
                                  <path
                                    fillRule="evenodd"
                                    clipRule="evenodd"
                                    d="M23.7288 9.86667H12.2034V14.5333H18.7119C18.4407 16 17.6271 17.2 16.2712 18.1333C15.1864 18.8 13.8305 19.3333 12.2034 19.3333C9.08475 19.3333 6.37288 17.2 5.42373 14.4C5.15254 13.6 5.01695 12.8 5.01695 12C5.01695 11.2 5.15254 10.4 5.42373 9.73333C6.37288 6.93333 9.08475 4.8 12.2034 4.8C13.9661 4.8 15.5932 5.46667 16.8136 6.53333L20.339 3.06667C18.3051 1.2 15.4576 0 12.2034 0C7.45763 0 3.25424 2.66667 1.35593 6.66667C0.40678 8.26667 0 10 0 12C0 14 0.40678 15.7333 1.35593 17.3333C3.38983 21.2 7.45763 24 12.2034 24C15.4576 24 18.3051 22.9333 20.339 21.0667C22.6441 18.9333 24 15.8667 24 12.2667C23.8644 11.4667 23.8644 10.6667 23.7288 9.86667Z"
                                    fill="white"
                                  />
                                </svg>
                              </a>
                              <a
                                className="b-autch-social__item b-autch-social__item--steam"
                                href="/login?service=steam"
                              >
                                <svg
                                  width={28}
                                  height={22}
                                  viewBox="0 0 28 22"
                                  fill="none"
                                  xmlns="http://www.w3.org/2000/svg"
                                >
                                  <path
                                    d="M15.1651 6.99241C15.1651 7.01933 15.1651 7.04625 15.1664 7.07152L11.2487 12.759C10.614 12.7302 9.97721 12.8414 9.37335 13.09C9.10716 13.1985 8.85773 13.3317 8.62369 13.4855L0.0209037 9.9463C0.020971 9.9463 -0.178159 13.2191 0.651418 15.6582L6.73311 18.166C7.03847 19.5298 7.97462 20.726 9.35576 21.3015C11.6155 22.245 14.221 21.1697 15.161 18.9104C15.4056 18.3199 15.5196 17.7005 15.5031 17.0824L21.1088 13.079C21.1541 13.0804 21.2009 13.0818 21.2462 13.0818C24.6008 13.0818 27.3274 10.3487 27.3274 6.99241C27.3274 3.63585 24.6008 0.905273 21.2462 0.905273C17.893 0.905273 15.1651 3.63585 15.1651 6.99241ZM14.2251 18.5176C13.4975 20.2632 11.4903 21.0914 9.74551 20.3648C8.9406 20.0297 8.3329 19.4158 7.98232 18.6838L9.96195 19.5037C11.2487 20.0394 12.7249 19.4296 13.2598 18.1441C13.7963 16.8572 13.1876 15.3794 11.9016 14.8438L9.85517 13.9964C10.6448 13.697 11.5425 13.686 12.3812 14.0349C13.2268 14.3865 13.8829 15.0484 14.2306 15.8945C14.5783 16.7405 14.5769 17.6744 14.2251 18.5176V18.5176ZM21.2462 11.0491C19.013 11.0491 17.1949 9.22939 17.1949 6.99241C17.1949 4.75722 19.013 2.93693 21.2462 2.93693C23.4808 2.93693 25.2989 4.75722 25.2989 6.99241C25.2989 9.22939 23.4808 11.0491 21.2462 11.0491ZM18.2105 6.98623C18.2105 5.30342 19.5737 3.93854 21.2531 3.93854C22.9338 3.93854 24.2971 5.30342 24.2971 6.98623C24.2971 8.66919 22.9338 10.0328 21.2531 10.0328C19.5737 10.0328 18.2105 8.66919 18.2105 6.98623V6.98623Z"
                                    fill="white"
                                  />
                                </svg>
                              </a>
                            </div>
                          </div>
                        </div>
                        <div className="js-tab-content" data-tab-index={2}>
                          <form
                            className="b-form b-form--signup js-form js-form-reg"
                            method="get"
                            action="/"
                          >
                            <input
                              type="hidden"
                              name="_csrf"
                              defaultValue="b09OaVkwQXhaIDouA1gTNgd.HzY4fS1OMDcROQEBCkAwDSs2NmcuNQ=="
                            />{" "}
                            <input type="hidden" name="referrer" />
                            <meta name="csrf-param" content="_csrf" />
                            <meta
                              name="csrf-token"
                              content="b09OaVkwQXhaIDouA1gTNgd.HzY4fS1OMDcROQEBCkAwDSs2NmcuNQ=="
                            />
                            <div className="b-form__message js-message" />
                            <div className="b-form__input">
                              <input
                                type="text"
                                id="registrationform-username"
                                className="input input--lg input--ta-cent"
                                name="RegistrationForm[username]"
                                data-validate="email"
                              />{" "}
                              <label>E-mail</label>
                              <div className="b-form__input-message" />
                            </div>
                            <div className="b-form__input">
                              <input
                                type="password"
                                id="registrationform-password"
                                className="input input--lg input--ta-cent"
                                name="RegistrationForm[password]"
                                data-validate="password"
                              />
                              <div className="b-form__input-message" />
                              <label>Пароль</label>
                            </div>
                            <div className="b-form__input b-form__input--agreement js-form-input">
                              <label className="b-checkbox b-checkbox--small">
                                <input
                                  className="js-agreement"
                                  type="checkbox"
                                  // defaultChecked=""
                                  defaultChecked
                                />
                                <span />
                                <div className="b-checkbox__label">
                                  Я&nbsp;даю согласие на&nbsp;обработку моих{" "}
                                  <a href="/documents/agreement" target="_blank">
                                    персональных данных
                                  </a>
                                </div>
                              </label>
                              <div
                                className="b-form__input-message js-form-input-mess"
                                style={{ display: "none" }}
                              >
                                Для завершения регистрации необходимо согласиться с
                                обработкой персональных данных
                              </div>
                            </div>
                            <button type="submit" className="btn btn--primary">
                              зарегистрироваться
                            </button>
                          </form>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div
            className="b-popup js-modal hide b-popup--change-comment"
            data-modal="change-comment"
          >
            <span className="b-popup__overlay" />
            <div className="b-popup__container js-custom-scrollbar">
              <div className="b-popup__content js-modal-content">
                <div className="b-comment-change-popup js-comment-change-popup">
                  <button
                    className="b-comment-change-popup__item js-comment-edit"
                    type="button"
                  >
                    Редактировать
                  </button>
                  <button
                    className="b-comment-change-popup__item js-comment-delete"
                    type="button"
                  >
                    Удалить
                  </button>
                  <button
                    className="b-comment-change-popup__close js-menu-close"
                    type="button"
                  >
                    Отменить{" "}
                  </button>
                </div>
              </div>
            </div>
          </div>
          <div className="b-preloader js-preloader" hidden >
            <img src="/images/preloader.svg" alt="Gabestore" />
          </div>
        </div>
      </header>
    </>
  )
}
